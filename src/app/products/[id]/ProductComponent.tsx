...
import {type ProductResponse} from "@/types/SingleProduct";
...
function ProductComponent({org_id, product_id}: { org_id: string, product_id: string }) {
  ...
  const item = items.find((uniqueProduct: Item) => uniqueProduct.id === product_id)
              ...
              {/*text*/}
                <div className={'flex-1 flex flex-col gap-4'}>
                    ...
                    <div className={'flex flex-col relative pb-2 justify-between'}>
                        ...
                        <span
                            className={'font-medium text-[1.9rem]'}>{USDollar.format(Number(product.current_price))}</span>
                    </div>
